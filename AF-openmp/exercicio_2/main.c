...
void init_matrix(double* m, int rows, int columns) {
    // #pragma omp parallel for schedule(guided) - ORIGINAL
...
void mult_matrix(double* out, double* left, double *right, 
    ...
    // #pragma omp parallel for schedule(dynamic, 1) - ORIGINAL
    for (i = 0; i < rows_left; ++i) {
        for (j = 0; j < cols_right; ++j) {
            out[i*cols_right+j] = 0;
            // #pragma omp parallel for firstprivate(i, j) schedule(guided) - ORIGINAL
